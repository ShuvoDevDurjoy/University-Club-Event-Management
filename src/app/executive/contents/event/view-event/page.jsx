...
const page = () => {
  ...
  const fetchEvents = async () => {
    try {
      const response = await axiosClient.get("/student/events");
              ...
              {events.upcoming.map((event, index) => {
                return (
                  ...
                  >
                    ...
                    <Link className="text-xl text-center px-5 py-3 w-full block bg-yellow-500 rounded-[5px] border-[2px] border-white" href={`/executive/contents/event/edit-event/${event.id}`}>Edit Event</Link>
                  </div>
                );
              })}
            </div>
